...
def carregar_dados_cadunico(caminho_arquivo: str = "data/cadunico.csv") -> pd.DataFrame:
    """
    ...
    try:
        ...
        logger.info(f"Dados do CadÚnico carregados: {df.shape[0]} registros, {df.shape[1]} colunas")
        return df
    except FileNotFoundError:
        logger.warning(f"Arquivo {caminho_arquivo} não encontrado. Gerando dados fictícios...")
...
def carregar_dados_bolsa_familia(caminho_arquivo: str = "data/bolsa_familia.csv") -> pd.DataFrame:
    """
    ...
    try:
        df = pd.read_csv(caminho_arquivo, encoding='utf-8')
        logger.info(f"Dados do Bolsa Família carregados: {df.shape[0]} registros, {df.shape[1]} colunas")
        return df
    except FileNotFoundError:
        logger.warning(f"Arquivo {caminho_arquivo} não encontrado. Gerando dados fictícios...")
...
def gerar_dados_ficticios_cadunico(n_registros: int = 1000) -> pd.DataFrame:
    ...
    data = {
        ...
        'escolaridade': np.random.choice([0, 1, 2, 3, 4, 5], n_registros),  # 0=analfabeto, 5=superior
        ...
        'situacao_trabalho': np.random.choice([0, 1, 2], n_registros),  # 0=desempregado, 1=informal, 2=formal
        'tipo_moradia': np.random.choice([1, 2, 3, 4], n_registros),  # 1=própria, 2=alugada, 3=cedida, 4=ocupação
...
def gerar_dados_ficticios_bolsa_familia(n_registros: int = 800) -> pd.DataFrame:
    """
    ...
    nis_cadunico = [f"{np.random.randint(10000000000, 99999999999)}" for _ in range(1000)]
    nis_beneficiarios = np.random.choice(nis_cadunico, n_registros, replace=False)
...
def tratar_dados_faltantes(df: pd.DataFrame, estrategia: str = 'mediana') -> pd.DataFrame:
    """
    ...
    if estrategia == 'mediana':
        ...
        for col in df_tratado.columns:
            df_tratado[col].fillna(df_tratado[col].mode().iloc[0] if not df_tratado[col].mode().empty else 0, inplace=True)
...
def gerar_features_vulnerabilidade(df_cadunico: pd.DataFrame, df_bolsa_familia: pd.DataFrame = None) -> pd.DataFrame:
    """
    ...
    # Feature de vulnerabilidade por idade (crianças e idosos são mais vulneráveis)
    df_features['vulnerabilidade_idade'] = ((df_features['idade'] < 18) | (df_features['idade'] > 65)).astype(int)
    
    # Feature de infraestrutura (combinação de acesso à água e esgoto)
    df_features['infraestrutura_adequada'] = (df_features['acesso_agua'] & df_features['acesso_esgoto']).astype(int)
    
    # Feature de escolaridade baixa
    df_features['escolaridade_baixa'] = (df_features['escolaridade'] <= 2).astype(int)
    
    # Feature de desemprego/informalidade
    df_features['situacao_trabalho_precaria'] = (df_features['situacao_trabalho'] <= 1).astype(int)
    
    # Feature de superlotação (muitas pessoas por família)
    df_features['superlotacao'] = (df_features['qtd_pessoas_familia'] > 5).astype(int)
    ...
    if df_bolsa_familia is not None:
        ...
        df_features['recebe_bolsa_familia'] = df_features['nis'].isin(beneficiarios_ativos).astype(int)
    ...
    df_features['renda_per_capita_norm'] = (df_features['renda_per_capita'] - df_features['renda_per_capita'].mean()) / df_features['renda_per_capita'].std()
    ...
    logger.info(f"Features de vulnerabilidade geradas para {len(df_features)} registros")
    return df_features


def consolidar_dados(df_cadunico: pd.DataFrame, df_bolsa_familia: pd.DataFrame) -> pd.DataFrame:
    """
...
def preparar_dados_para_ml(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    logger.info(f"Dados preparados para ML: {X.shape[0]} amostras, {X.shape[1]} features")
    return X, y
